...
var editEquipoModal = new bootstrap.Modal(
  document.getElementById("editEquipoModal"),
  {}
);

function editarEquipo(id) {
  var baseURL = APP_URL + "/admin/equipo/getEquipo";
  const data = { id_eq: id };
...
function update(event) {
  ...
  var baseURL = APP_URL + "/admin/equipo/upEquipo";
...
// eliminar
var deleteEquipoModal = new bootstrap.Modal(
  document.getElementById("deleteEquipoModal"),
  {}
);
...
function eliminarEquipo(id) {
  ...
  axios.post(baseURL, data).then((response) => {
    ...
    document.getElementById("lPuesto").textContent =
...
function deleteEq(event) {
  event.preventDefault();
  var baseURL = APP_URL + "/admin/equipo/delEquipo";
  let encabezados = new Headers();
  let form = new FormData(this);
  axios.post(baseURL, form, { encabezados }).then((response) => {
    if (response.data.status) {
...
//status
var statusEquipoModel = new bootstrap.Modal(
  document.getElementById("statusEquipoModel"),
  {}
);
...
function estadoEq(id) {
  ...
  axios.post(baseURL, data).then((response) => {
    if (response.data.estado == 1) {
      document.getElementById("titModel").textContent =
    ...
    document.getElementById("sNombre").textContent =
      "Nombre: " + response.data.nombre;
    document.getElementById("sPuesto").textContent =
      "Puesto: " + response.data.puesto;
...
function changeEq(event) {
  event.preventDefault();
  var baseURL = APP_URL + "/admin/equipo/statusEquipo";
  let encabezados = new Headers();
  let form = new FormData(this);
  axios.post(baseURL, form, { encabezados }).then((response) => {
    if (response.data.status) {
